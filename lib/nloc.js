  ...
  if(options && options.target) {
    ...
    if(exports.checkPath(options.target)) {
      ...
      if(fs.lstatSync(options.target).isDirectory()) {
        filesInTarget = exports.findInPath(options.target, {
          recursive: true
        });
      } else {
        filesInTarget[options.target] = exports.getFileObj(options.target);
      }

      for(var key in filesInTarget) {
        filesInTarget[key].analyze = exports.analyzeFile(key, options);
      }
      if(_.isFunction(done)) {
        done(null, filesInTarget);
      }
